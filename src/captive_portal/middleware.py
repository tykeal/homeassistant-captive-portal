...
async def rbac_enforcer(request: Request, action: str | None = None) -> None:
    """Enforce RBAC permissions for request (placeholder implementation).

    Args:
        request: FastAPI request
        action: Action to authorize (or read from X-Action header)
    ...
    if not is_allowed(role, action):
        raise HTTPException(
            status_code=403, detail={"error": "forbidden", "code": "RBAC_FORBIDDEN"}
        )
