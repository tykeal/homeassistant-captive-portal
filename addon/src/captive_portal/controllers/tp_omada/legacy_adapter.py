...
"""Legacy TP-Omada controller adapter for grant authorization and revocation."""
...
class OmadaLegacyAdapter:
    """Legacy adapter for TP-Omada controller operations.

    Attributes:
        client: OmadaLegacyClient instance for HTTP communication
        site_id: Omada site identifier (e.g., "Default")
    """

    def __init__(self, client: OmadaLegacyClient, site_id: str = "Default") -> None:
        """Initialize Omada adapter.

        Args:
            client: Initialized OmadaLegacyClient
            site_id: Omada site identifier (default: "Default")
        """
        self.client = client
        self.site_id = site_id

    async def _post_with_managed_client(
        self,
        endpoint: str,
        payload: dict[str, Any],
    ) -> dict[str, Any]:
        """POST to the legacy client, opening a request-local session if needed.

        Args:
            endpoint: Legacy controller endpoint.
            payload: JSON payload.

        Returns:
            Parsed legacy controller response.
        """
        if self.client._client is not None or "post_with_retry" in self.client.__dict__:
    ...
    async def revoke(
        ...
        endpoint = f"/{self.client.controller_id}/api/v2/hotspot/extPortal/auth"
        await self._post_with_managed_client(endpoint, payload)
