...
- [X] T026 [P] Add data directory validation in `src/captive_portal/config/settings.py` `load()` method — after resolving effective `db_path`, validate that the parent directory exists and is writable; raise a descriptive `RuntimeError` with the path and permission details if validation fails (edge case from spec)
