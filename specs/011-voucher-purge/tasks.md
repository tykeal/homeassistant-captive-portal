...
**Tests**: TDD is NON-NEGOTIABLE per project constitution. All test tasks MUST be written first and verified to fail before implementation begins.
...
- [ ] T013 [US1] Add `count_purgeable(cutoff: datetime) -> int`, `get_purgeable_codes(cutoff: datetime) -> list[str]`, and `purge(cutoff: datetime) -> int` methods to `VoucherRepository` using batch SQL: `SELECT COUNT(*) ... WHERE status IN ('expired', 'revoked') AND status_changed_utc < :cutoff`, `SELECT code ... WHERE status IN ('expired', 'revoked') AND status_changed_utc < :cutoff`, and `DELETE FROM voucher WHERE status IN ('expired', 'revoked') AND status_changed_utc < :cutoff` — in `addon/src/captive_portal/persistence/repositories.py`
...
- [ ] T015 [US1] Create `VoucherPurgeService` class in `addon/src/captive_portal/services/voucher_purge_service.py` with: `__init__(voucher_repo, grant_repo, audit_service, retention_days=30)`, `auto_purge() -> int` (calculates cutoff, queries purgeable codes, nullifies grant references, deletes vouchers, logs audit entry), `count_purgeable(min_age_days: int) -> int`, and `manual_purge(min_age_days: int, actor: str) -> int`; include SPDX header and full docstrings
- [ ] T016 [US1] Wire `VoucherPurgeService.auto_purge()` call into `get_vouchers` GET handler, executing immediately after the existing `expire_stale_vouchers()` call, in `addon/src/captive_portal/api/routes/vouchers_ui.py`; instantiate `VoucherPurgeService` with repository and audit service dependencies
