...
- The logout mechanism MUST invoke the existing `/api/admin/auth/logout` endpoint to terminate the admin session. Any current mismatches (for example, admin templates or navigation forms posting to a different path such as `/api/admin/logout`) MUST be corrected so that the UI-level logout flow calls this endpoint. A dedicated HTML logout handler/route (for example, `/admin/logout`) MUST be provided that calls `/api/admin/auth/logout` and then redirects the browser to the admin login page, so that clicking "Logout" always results in a navigation back to the login screen, including when JavaScript is disabled. The backend API contract for `/api/admin/auth/logout` itself is assumed to remain a JSON response (no redirect).
