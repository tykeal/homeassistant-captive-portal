...
- [ ] T003 [P] Write failing unit tests for admin app lifespan Omada wiring in `tests/unit/test_app_lifespan_omada.py` — test that when Omada settings are configured (`omada_controller_url` non-empty): (a) `OmadaClient` is constructed with correct params (`base_url`, `controller_id`, `username`, `password`, `verify_ssl`), (b) `OmadaAdapter` is constructed with the client and `site_name`, (c) both stored on `app.state.omada_client` and `app.state.omada_adapter`, (d) no network I/O occurs during startup (mock to verify no `__aenter__` called); test that when Omada is NOT configured (empty URL): (e) `app.state.omada_client` is `None`, (f) `app.state.omada_adapter` is `None`, (g) no errors logged (RED 🔴)

- [ ] T004 [P] Write failing unit tests for guest app lifespan Omada wiring in `tests/unit/test_guest_app_lifespan_omada.py` — same scenarios as T003 but targeting `guest_app.py` lifespan: configured → client+adapter on state, not configured → `None` on state, no startup network I/O; verify guest app creates its own independent client/adapter instances (not shared with admin app) (RED 🔴)

- [ ] T005 [P] Write failing unit test for `get_omada_adapter` dependency function in `tests/unit/controllers/tp_omada/test_omada_dependencies.py` — verify: (a) when `app.state.omada_adapter` exists, returns the `OmadaAdapter` instance, (b) when `app.state.omada_adapter` is `None`, returns `None`, (c) when `omada_adapter` attribute does not exist on state (not set at all), returns `None` gracefully (RED 🔴)
...
- [ ] T007 Add 6 Omada fields to `AppSettings` in `addon/src/captive_portal/config/settings.py` — add field declarations with defaults per data-model.md (`omada_controller_url: str = ""`, `omada_username: str = ""`, `omada_password: str = ""`, `omada_site_name: str = "Default"`, `omada_controller_id: str = ""`, `omada_verify_ssl: bool = True`); extend `_ADDON_OPTION_MAP` with 6 entries (`"omada_controller_url": "omada_controller_url"`, etc.); extend `_ENV_VAR_MAP` with 6 entries (`"CP_OMADA_CONTROLLER_URL": "omada_controller_url"`, etc.); extend `load()` resolution loop to handle these fields including boolean coercion for `omada_verify_ssl`; extend `log_effective()` to log all 6 fields, with `omada_password` shown as `"(set)"`/`"(not set)"` — never the actual value; add `omada_configured` property that returns `bool(self.omada_controller_url)` (GREEN 🟢 — T002 passes)
...
- [ ] T010 Wire `OmadaClient` and `OmadaAdapter` into admin app lifespan in `addon/src/captive_portal/app.py` — in `_make_lifespan()` startup section: if `settings.omada_controller_url` is non-empty, construct `OmadaClient(base_url=settings.omada_controller_url, controller_id=settings.omada_controller_id, username=settings.omada_username, password=settings.omada_password, verify_ssl=settings.omada_verify_ssl)` and `OmadaAdapter(client=client, site_id=settings.omada_site_name)`, store on `app.state.omada_client` and `app.state.omada_adapter`, log "OmadaClient initialized for {url}"; if URL is empty, set both to `None` and log info "Omada controller not configured — controller calls will be skipped"; no `__aenter__` call at startup (lazy init per research R1); no shutdown cleanup needed since route handlers use per-operation `async with client:` (GREEN 🟢 — T003 passes)

- [ ] T011 Wire `OmadaClient` and `OmadaAdapter` into guest app lifespan in `addon/src/captive_portal/guest_app.py` — same pattern as T010: construct and store on `app.state` when configured, set to `None` when not; independent instances from admin app; log Omada initialization status (GREEN 🟢 — T004 passes)

- [ ] T012 Create `get_omada_adapter` dependency function in `addon/src/captive_portal/controllers/tp_omada/dependencies.py` — function signature `def get_omada_adapter(request: Request) -> OmadaAdapter | None`; returns `getattr(request.app.state, "omada_adapter", None)`; add SPDX header and docstring; this function is used as `Depends(get_omada_adapter)` in route handlers that need controller access (GREEN 🟢 — T005 passes)
...
- [ ] T015 [US1] Wire controller authorize call into guest authorization flow in `addon/src/captive_portal/api/routes/guest_portal.py` — after grant creation (PENDING status): access `omada_adapter` via `get_omada_adapter` dependency or `request.app.state`; if adapter is not `None`: enter `async with adapter.client:` context, call `await adapter.authorize(mac=mac_address, expires_at=grant.end_utc)`, on success update `grant.status = GrantStatus.ACTIVE` and `grant.controller_grant_id = result["grant_id"]`, commit to database; on `OmadaClientError` or `OmadaRetryExhaustedError`: update `grant.status = GrantStatus.FAILED`, log error with structured logging, create audit log entry for failure, return user-friendly error message to guest; if adapter is `None`: set `grant.status = GrantStatus.ACTIVE` directly (existing behavior preserved per FR-013); commit grant status change (GREEN 🟢 — T013, T014 pass)
